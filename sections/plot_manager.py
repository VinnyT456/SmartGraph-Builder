...
class PlotManager:
    ...
    def update_x_axis_title(self,title):
        ...
        self.db.update({"axis-title":current_title})
    ...
    def update_title(self,title):
        self.db.update({"title":title})

    def update_legend(self,parameter,new_value):
        self.db.update(lambda db: db["legend"].update({parameter:new_value}) or db)

    def update_seaborn_legend(self,parameter,new_value):
        self.db.update(lambda db: db["legend"]["seaborn_legends"].update({parameter:new_value}) or db)

    def update_grid(self,parameter,new_value):
        self.db.update(lambda db: db["grid"].update({parameter:new_value}) or db)

    def update_hue(self,hue):
        self.db.update({"hue":hue})

    def update_style(self,style):
        self.db.update({"style":style})

    def update_size(self,size):
        self.db.update({"size":size})

    def update_sizes(self,sizes):
        self.db.update({"sizes":sizes})

    def update_palette(self,palette):
        self.db.update({"palette":palette})

    def update_alpha(self,alpha):
        self.db.update({"alpha":alpha})

    def update_marker(self,marker):
        self.db.update({"marker":marker})

    def update_s(self,s):
        self.db.update({"s":s})

    def update_edgecolors(self,edgecolors):
        self.db.update({"edgecolors":edgecolors})
