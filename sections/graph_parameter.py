...
from sections.catalog.plots import SEABORN_PLOTS, get_plot_spec
...
class graph_parameter_table(QWidget):
    ...
    def update_parameter_buttons(self):
        ...
        def handle_button_function(name, function):
            ...
            SPECIAL_DIALOGS = {"x-axis", "y-axis"}

            # Create dialog if it doesn't exist
            name_lower = name.lower()
            if name not in self.dialogs:
                if name_lower in SPECIAL_DIALOGS:
                    instance = function(get_plot_spec(selected_graph).parameters, selected_graph, self.graph_display)
                else:
            ...
            for col in range(2):
                ...
                if button_function:
                    button.clicked.connect(
                        lambda checked=False, name=button_name, func=button_function: (
                            handle_button_function(name, func)
                        )
                    )
...
class select_graph_window(QDialog):
    def __init__(self, available_graphs, graph_parameter_table, select_graph_button):
        ...
        container_widget_layout.addSpacing(5)
        container_widget_layout.addWidget(
            self.graph_pages_section, alignment=Qt.AlignmentFlag.AlignCenter
        )
        container_widget_layout.addSpacing(5)
    ...
    def create_pages_section(self):
        ...
        for i in range(int(len(self.available_graphs) / 3)):
            ...
            graph_pages_section_layout.addWidget(
                dot, alignment=Qt.AlignmentFlag.AlignVCenter
            )
    ...
    def add_graph_buttons(self):
        ...
        for i in range(len(self.available_graphs)):
            ...
            graph_button.setObjectName(self.available_graphs[i])
            ...
            graph_button.clicked.connect(
                lambda checked, b=graph_button: self.update_graph_image(b)
            )

            graph_button_layout = QVBoxLayout(graph_button)
            graph_button_layout.addWidget(
                graph_label, alignment=Qt.AlignmentFlag.AlignCenter
            )
...
class select_graph(QPushButton):
    def __init__(self, graph_parameter_table):
        ...
        self.graph_window = select_graph_window(
            list(SEABORN_PLOTS.keys()), self.graph_parameter_table, self
        )
