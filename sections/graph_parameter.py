...
class x_axis_button(QWidget):
    def __init__(self):
        ...
        self.select_x_value_window = QDialog()
        self.select_x_value_window.setObjectName("x_value")
        self.select_x_value_window.setStyleSheet("""
            QDialog#x_value {
        ...
        """)
        self.select_x_value_window.exec()

    def find_usable_columns(self):
        x_axis_data_type = SEABORN_PLOTS[select_graph].get("x-axis_data_type")
        if (x_axis_data_type):
            pass

class y_axis_button(QWidget):
    def __init__(self):
        super().__init__()

        self.dataset = pd.read_csv("./dataset/user_dataset.csv")
        
        self.select_y_value_window = QDialog()
        self.select_y_value_window.setObjectName("y_value")
        self.select_y_value_window.setStyleSheet("""
            QDialog#y_value {
               background: qlineargradient(
                    x1: 0, y1: 1, 
                    x2: 0, y2: 0,
        ...
        """)
        self.select_y_value_window.exec()
    ...
    def update_parameter_buttons(self):
        ...
        for row in range(7):
            for col in range(2):
                ...
                if (button_function):
                    button.clicked.connect(lambda checked=False, func=button_function: func())
